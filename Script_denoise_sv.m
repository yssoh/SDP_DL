% Script for denoising using polyhedral norms
% Body of the code
...
for rr = 1 : N_REPEATS
    ...
    for jj = 1 : l
        ...
        DICTPATH = strcat('instances/',DATASET,'/output/r',int2str(dictround),'sv_q',int2str(q),'r',int2str(r),'.mat');
        load(DICTPATH,'A')
        fprintf('Done.\n')
        ...
        for kk = 1 : N_REGS
            ...
            if parmode == 1
                parfor ii = 1 : n
                    Y_DENOISE(:,ii) = A*svlasso(A,Y_TEST_NOISY(:,ii),q,'reg',REG);
                end
            else
                for ii = 1 : n
                    Y_DENOISE(:,ii) = A*svlasso(A,Y_TEST_NOISY(:,ii),q,'reg',REG);
        ...
        DICTPATH = strcat('instances/',DATASET,'/testerr/r',int2str(dictround),'n',int2str(rr),'err_sv_q',int2str(q),'r',int2str(r),'.mat');
